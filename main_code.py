...
dropout = 0.750
batch_size = 50
data = pd.read_csv('train_rot_trans.csv')
...
#weight and bias for fully connected layer
weight_fc2 = param_weight([1024, count_of_labels])
bias_fc2 = bias_feature([count_of_labels])

#softmax layer to take maximum probablity value from last output
y = tf.nn.softmax(tf.matmul(h_fc1_drop, weight_fc2) + bias_fc2)

# cost function
cross_entropy = -tf.reduce_sum(y_*tf.log(y))

# optimisation function fro early convergence
...
np.savetxt('predicted_output.csv',np.c_[range(1,len(test_data)+1),predicted_digits], delimiter=',',header = 'ImageId,Label', comments = '', 
fmt='%d')
sess.close()